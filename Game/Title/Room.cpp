...
void Room::Init()
{
	...
	model_ = new Model();
	model_->Initialize("Resources/room", "room.obj", color);
	modelFront_ = new Model();
	modelFront_->Initialize("Resources/room", "roomFront.obj", color);
	shutter_ = new Model();
	shutter_->Initialize("Resources/room", "shutter.obj", color);
...
void Room::Draw(Camera* camera)
{
	...
	modelFront_->Draw(worldTransform_, texture_, camera, color);
}
